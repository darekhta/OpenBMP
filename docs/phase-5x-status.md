...
| 5.X.E — Vocabulary migration | ✅ Done (scenario lint + CI tripwire + kernel typed pending queues + workspace-wide rack signature migration + zero deprecation warnings on `cargo build --workspace`. Legacy `EventAction` retained as a deprecated bridge enum until its physical removal in a follow-up; every production call site uses `MissionAction` / `ScenarioScriptAction`) | `Phase 5.X.E pre-work + 5.X.E (2-7/N)` |
