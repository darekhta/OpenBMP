...
| Plume similarity substrate | `crates/openbmp-plume/src/lib.rs` (`PlumeState`, `PlumeNozzle`, `PlumeFreestream`, `PlumeClusterGeometry`); `crates/openbmp-runner/src/plume.rs` | **Partial WP-15.1.** Computes NPR, exit-pressure ratio, `C_T`, documented total-thrust/reference-area momentum-flux ratio, Prandtl-Meyer initial turn angle, reduced cluster-merge distance, and PIFS onset from coordinate-free nozzle/freestream/geometry scalars. Point-mass solid-motor scenarios can opt into telemetry via `[aero.plume]` and derive single-engine count, nozzle exit area, and zero spacing from the motor; rigid-body thermochemical liquid-engine scenarios can also emit plume telemetry from live engine mass-flow snapshots, with active merge spacing derived from engine mount points. Off-byte-golden coverage now proves absent `[aero.plume]` leaves point-mass and rigid-body telemetry bytes unchanged. Rigid full derived cluster-geometry assembly remains pending. |
...
- **implementation_status:** partial. The L2 crate skeleton and coordinate-free
  ...
  `REQ-PLUME-004` / `V-PLUME-004`. Canonical byte-identity for the absent
  `[aero.plume]` path is traced by `REQ-PLUME-005` / `V-PLUME-005`.
  Point-mass solid-motor plume geometry now derives single-engine count, exit
  area, and zero spacing from the motor and is traced by `REQ-PLUME-006` /
  `V-PLUME-006`. The `momentum_flux_ratio` diagnostic now follows the
  documented total-thrust-over-`rho V^2 S_ref` similarity equation and is
  traced by `REQ-PLUME-007` / `V-PLUME-007`. Rigid full derived multi-engine
  geometry and broader liquid-engine calibration remain to complete the WP.
