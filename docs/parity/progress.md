...
`PinesLongitudePolynomials`, and `TideSystem`: the first degree-2/order-2
...
factors, a bounded `cos(mλ)`/`sin(mλ)` recurrence table, a singularity-free
...
recurrence table, and default-zero coefficient-slot iteration, rebuilds the current
