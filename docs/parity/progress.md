...
against finite-difference columns; `src/shooting.rs` seeds and evaluates
fixed-duration M-segment continuity defects with a block-bidiagonal
`[STM_i, -I]` Jacobian (`REQ-TRAJOPT-002`). Missing: the solve loop,
cross-tier regression tolerance table, STM-vs-complex-step gate, compile-fail
...
3. **Finish WP-07.1** (multiple-shooting solve loop, STM-vs-complex-step gate,
   no-surface-coordinate tripwire, and path/box penalties) or use the implemented
