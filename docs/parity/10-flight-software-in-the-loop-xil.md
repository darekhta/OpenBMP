...
Verified by reading the files below at the program baseline plus the current parity implementation slices. The honest summary: **the schema, validators, generic transports, bridge-level lockstep master, opt-in runner in-process/TCP/Unix-loopback/external-process transport subset, bridge packet expansion, actuator-stream determinism gates, XIL evidence hooks, realtime pacer, first forward AFTS substrate, no-std PIL byte/mailbox/framed/byte-poll endpoint shim, safe `PilFirmware` target wrapper, integer PIL WCET budget helper, deterministic Renode socket-UART script renderer, ELF symbol resolver, GDB mailbox/RSP planner/session, and host raw-frame socket client exist; live PIL/cFS target execution does not.** OpenBMP is on Tier 0 ("algorithm-in-the-loop") with partial Tier 1/Tier 3/A0 substrate in place.

| Capability | Where | State |
|---|---|---|
| Lockstep **wire schema** | `crates/openbmp-bridge/src/packet.rs` | `SensorPacket`, `ActuatorCommandPacket`, `StepAckPacket`/`StepAckStatus`, `BridgeHelloPacket`, `BridgeMessage`, `BridgeFaultPacket`/`BridgeFaultCode`, `PROTOCOL_VERSION = 4`. Complete and `serde`-derived. The packet surface now compiles in `openbmp-bridge --no-default-features` for `thumbv7em-none-eabihf`; host transports/fault ports remain `std`-gated. |
| **Codec + framing** | `crates/openbmp-bridge/src/codec.rs` | `encode`/`decode` (`postcard`), `encode_into` for caller-owned target buffers, `frame`/`deframe` (`u32`-LE length prefix, concatenation-safe), and `frame_into` for caller-owned framed output. Complete. The codec participates in the no-std embedded bridge check so target firmware can reuse the same frame bytes without pulling in sockets or XIL ports. |
...
| **FC `no_std` posture** | `crates/openbmp-fc/src/lib.rs`, `crates/openbmp-fc/src/step.rs`, `crates/openbmp-pil/src/lib.rs`, `crates/openbmp-runner/src/pil.rs`, `Cargo.toml` | `#![cfg_attr(not(feature = "std"), no_std)]`, `#![forbid(unsafe_code)]`, `extern crate alloc`; builds for `thumbv7em-none-eabihf` (`docs/HAL.md` verification hooks). `openbmp-fc::fc_step` now exposes a hardware-portable topic-level `FcStepInput` -> `FcStepOutput` boundary that publishes sensor topics, dispatches one controller tick, and captures actuator/health/status outputs without adding bridge/runner dependencies; `openbmp-bridge` now exposes target-compatible packet/codec/validator pieces with host transports and XIL ports gated behind `std`; `openbmp-pil` provides the target-checkable byte shim, `repr(C)` mailbox, `PilMailboxLayout` offset report, `PilFrameIo` complete-frame endpoint, fixed-buffer `PilFramePump` byte-stream pump, `PilByteIo` poll loop, safe `PilFirmware` owner for one controller/mailbox/pump/read-scratch set, advisory `OPENBMP_PIL_MAILBOX` / `openbmp_pil_step_mailbox` symbol contract, and integer `PilWcetBudget` / `PilWcetReport` helpers for cycle-approximate deadline evidence; `openbmp-runner` owns the host bridge-packet adapter outside the FC crate, has a unit gate proving the framed PIL endpoint matches the runner adapter's encoded command bytes under the host oracle, exposes a validated `RenodePilPlan` renderer for a socket-backed UART `.resc` coupling script, exposes `GdbPilSymbolBinding` ELF symbol resolution plus `GdbPilMailboxPlan`/`GdbMemoryRead`/`GdbMemoryWrite`/`GdbBreakpointCommand` helpers that map a resolved mailbox address into GDB Remote Serial Protocol memory and breakpoint packets, exposes `GdbRspSession` over caller-owned streams with RSP ack/checksum validation, target-error handling, memory read/write, breakpoint/continue, mailbox sensor loading, and mailbox status/command reads, and exposes `PilFrameStream`/`RenodeSocketPilClient` to send length-prefixed sensor frames and validate command frames over that raw socket. **Remaining PIL work:** downstream-exported linked firmware image, live Renode target execution, real UART driver, actuator-stream equivalence, and measured WCET evidence. |
...
| AFTS forward containment substrate | `crates/openbmp-afts/src/lib.rs`, `crates/openbmp-afts/src/error.rs`, `crates/openbmp-runner/src/afts.rs`, `crates/openbmp-scenario/src/document.rs`, `crates/openbmp-testkit/tests/afts_no_inverse_compile_fail.rs` | **WP-10.6 substrate present.** `openbmp-afts` is a `no_std`-capable L2 crate depending only on `openbmp-core`, `openbmp-state`, and `openbmp-physics`. It exposes `IipPropagator` over OpenBMP ECI point-mass/rigid states using WGS84 symbolic constants, optional WGS84 uniform-rotation longitude correction, WGS84 sphere/ellipsoid `IipImpactSurface` selection, optional deterministic `ExponentialAtmosphericDrag`, `ContainmentPolygon` point-in-polygon checks, `ContainmentRule::keep_inside`/`keep_out`, scalar `CorridorRule` checks over altitude/speed/flight-path-angle samples, geospatial `GateRule` crossing checks over successive IIP samples, red/green `ZoneRule` semantics with red-zone priority and green-zone union checks, and a latched `AftsMonitor` decision with the first firing rule id. Schema v3 parses `[afts.propagator]` plus `[afts]` keep-inside, keep-out, corridor, gate, and zone rules with nonempty evidence strings, optional resolved/pinned `evidence_file`, fail-closed propagator/drag validation, fail-closed angle/antimeridian validation, finite inclusive corridor bounds, nondegenerate gate endpoints, and green/red zone colors. The runner samples telemetry-aligned truth states into `AftsRunReport` without changing kernel `StopReason`; CLI run output and SIL `EvidenceBundle` carry the sidecar report. Native `SilStimulation` can append AFTS zone rules in memory without changing package scenario files and records those stimuli in evidence. Tests cover radial IIP plus a non-radial rotating-Earth IIP from `data/afts/closed-form-iip-tolerance-v1.toml` against closed-form conic points, WGS84 Earth-rotation longitude correction, WGS84 ellipsoid geodetic latitude, exponential-drag IIP perturbation, hand-computed containment cases, keep-out firing, scalar corridor firing, gate crossing, green/red zone firing, latch behavior, no-impact horizon rejection, no FC dependency, AFTS parser validation/resolved-file hashing, runner-side latching including ellipsoid altitude, SIL AFTS zone stimulation, and a trybuild no-inverse tripwire for `solve_burn_to_reach`. This remains non-certified simulation evidence, not range-safety qualification. |
...
**Maturity verdict:** Tier 0 complete and honestly labelled. Tiers 1, 2, and 6 build directly on existing artifacts; Tier 5 builds on the already-`no_std` FC; Tiers 3–4 are net-new but self-contained; A0 AFTS now has a forward substrate plus runner/SIL evidence wiring, pinned evidence-file resolution, keep-in/keep-out polygons, scalar corridor checks, IIP gate crossings, green/red zone semantics, radial and non-radial closed-form conic IIP checks, WGS84 ellipsoid surface selection, optional exponential drag, and package-level SIL zone stimulation, but not certification scope.
...
NEW (proposed; placement justified in the WP that introduces it):
  ...
  openbmp-afts     L2   forward IIP-geo-containment MONITOR (rule table + ballistic IIP).
                        First substrate now ships; hardware-portable
                        (no_std-capable); MAY be consumed by openbmp-fc as a
                        monitor, so it MUST obey the FC portability lock.
```

> **Crate-boundary justifications (reviewed before building):**
> `openbmp-rt` is L7 (host orchestration) and is *never* a dependency of `openbmp-fc`/`openbmp-hal` — it paces the simulator loop, not the controller, so it cannot break the portability lock. `openbmp-afts` is placed at L2 (a physics-layer monitor over forward state) and is `no_std`-capable so the FC *may* host it as an independent string; it depends only on `openbmp-core`/`openbmp-state`/`openbmp-physics`. The first shipped substrate uses a fixed-step central-gravity IIP propagator plus keep-inside polygons, latch semantics, and a compile-fail no-inverse tripwire (§6). Neither crate adds an up-layer edge or an FC→simulator edge.
...
`openbmp-fmi` already loads the `.so` and probes the lifecycle/value-access smoke symbols (`crates/openbmp-fmi/src/lib.rs`); this WP now has typed Float64/Int32/UInt64/Clock value-ref calls, deterministic periodic input-clock activation at macro-step boundaries, `fmi3DoStep` early-return reporting, owned instantiate/init/terminate/free through `Fmi3CoSimulationInstance`, optional FMU-state snapshot/restore/free wrappers, a single-FMU early-return rollback policy, deterministic typed multi-FMU connection routing for Gauss-Seidel/Jacobi ordering, a checkpointed multi-FMU predictor/corrector macro-step that restores FMU states and master clocks after the predictor pass before seeding the corrector from predicted outputs, stored/deflated FMU archive reading plus `modelDescription.xml` instantiation-token, value-ref, and Clock metadata parsing (reusing `openbmp-models::FmuArchive`), safe FMU resource materialization, safe current-platform binary-entry inference for standard FMI 3 packages, a packaged point-mass importer smoke gate through `scripts/check_fmi_import_point_mass.py`, an OpenBMP-importer-vs-FMPy point-mass cross-check through `scripts/check_fmi_import_fmpy.py`, and pinned no-input Float64/Int32 Reference-FMU smokes through `scripts/check_fmi_reference_smoke.py`. The remaining importer-master work is complete clocked event scheduling and full external Reference-FMU matrix validation. The `unsafe extern "C"` FFI is confined to `openbmp-fmi` (a host-only crate, never a dep of `openbmp-fc`).
...
**Precondition partially met:** `openbmp-fc` is `#![cfg_attr(not(feature="std"), no_std)]`, `forbid(unsafe_code)`, `extern crate alloc`, and compiles for `thumbv7em-none-eabihf` today. The crate now factors `fc_step(FcStepInput) -> FcStepOutput`, a topic-level entry that publishes hardware-portable sensor samples, dispatches one controller tick, and captures actuator/health/status outputs while keeping `SensorPacket`/`ActuatorCommandPacket` adaptation outside `openbmp-fc`. `openbmp-bridge` packet/codec/validator modules now compile without default features for the same embedded target, while host transports, XIL ports, and bridge fault transforms stay behind `std`; `encode_into` and `frame_into` support caller-owned target buffers. `openbmp-pil` adds the target-checkable byte adapter, `repr(C)` `PilMailbox`, `PilMailboxLayout`, `PilFrameIo` complete-frame endpoint, fixed-buffer `PilFramePump`, and `PilByteIo` polling loop: decode `SensorPacket` bytes or length-prefixed frames, ingest arbitrary byte chunks, wait for complete frames, preserve queued and pending frames, call `fc_step`, convert captured effector/engine topics to `ActuatorCommandPacket`, encode command bytes into caller storage, and expose target-visible status/error codes plus concrete field offsets for a downstream known-symbol or UART-backed firmware wrapper. `openbmp-pil::PilFirmware` now owns one initialized controller plus mailbox, pump, and bounded read scratch behind safe Rust methods for mailbox stepping, complete-frame exchange, and byte-stream polling; `PilFirmwareSymbolContract` keeps the advisory mailbox/step symbol names paired with the concrete mailbox layout without using unsafe linker attributes; and `PilWcetBudget` / `PilWcetReport` provide integer instruction-count × CPI deadline evidence plumbing. `openbmp-runner` provides the host bridge-packet adapter through `FcRunner::step_bridge_packet`, a validated `RenodePilPlan` renderer that emits `mach create`, `machine LoadPlatformDescription`, `sysbus LoadELF`, `emulation CreateServerSocketTerminal ... false`, `connector Connect`, `emulation SetGlobalQuantum`, and optional `machine StartGdbServer` commands for a future socket-UART PIL run, `GdbPilSymbolBinding` helpers that resolve mailbox and step-entry addresses from target ELF32/ELF64 symtab/dynsym bytes or files, a `GdbPilMailboxPlan` that turns the resolved mailbox symbol address plus the target layout into deterministic GDB Remote Serial Protocol read/write/breakpoint packets and target-read decoders, `GdbRspSession` stream mechanics that perform checked RSP packet exchange, ack/checksum validation, target-error handling, memory reads/writes, breakpoint insert/remove, continue, mailbox sensor loading, and mailbox status/command reads, and a raw-frame `PilFrameStream` / `RenodeSocketPilClient` that sends encoded `SensorPacket` frames to that socket, decodes `ActuatorCommandPacket` frames, enforces payload limits, and validates command step/time before unblocking. Remaining PIL work is a downstream-exported linked target image, live Renode target execution, real UART transport binding, target equivalence, and measured WCET evidence.
...
- *Numeric:* integrate `r̈ = −GM_earth · r/‖r‖³` (+ optional drag) until the selected WGS84 sphere or ellipsoid surface is crossed; the surface intersection is the IIP. The shipped `openbmp-afts` substrate implements the fixed-step RK4 central-gravity path with WGS84 symbolic constants, WGS84 sphere/ellipsoid surface selection, opt-in WGS84 uniform-rotation longitude correction, and an optional deterministic exponential ballistic-drag perturbation. Earth constants are referenced symbolically as `GM_earth`, `J2`, `R_earth` (never written as literals — `00` §3.5).
...
- **current_state:** Partial implementation. `openbmp-models::FmuArchive` reads stored and deflated FMU ZIP entries, parses the FMI 3 root `instantiationToken`, required `valueReference` fields, supported `Float64`/`Int32`/`UInt64` scalar variables, direct FMI 3 `Clock` variables, and scalar-variable `clocks` dependencies; `openbmp-fmi` builds typed bindings/master plans, safely materializes FMU `resources/` entries, infers safe standard FMI binary entries for supported current-platform targets, owns co-simulation instantiation/init/termination/free through `Fmi3CoSimulationInstance`, calls typed `fmi3Set/GetFloat64`, `fmi3Set/GetInt32`, `fmi3Set/GetUInt64`, `fmi3Set/GetClock`, and `fmi3DoStep` with early-return reporting through host FFI, publishes deterministic periodic input-clock activations at single-FMU macro-step boundaries, exposes optional `fmi3Get/Set/FreeFMUState` wrappers, provides `Fmi3SingleFmuMaster` for one-FMU macro steps, can restore a pre-step FMU-state checkpoint on early return through `Fmi3RollbackPolicy::RestoreOnEarlyReturn`, provides `Fmi3MultiFmuMaster` for typed `Float64`/`UInt64` connection routing across existing FMU instances with deterministic Gauss-Seidel/Jacobi ordering, and can run a checkpointed multi-FMU predictor/corrector macro-step that restores FMU states and master clocks after the predictor pass before seeding the corrector from predicted outputs. `scripts/check_fmi_import_point_mass.py` packages the current-platform point-mass FMU, runs the OpenBMP importer path twice from fresh materialized binaries, byte-compares the CSV outputs, and validates the final sample against the native trace tolerance table; `scripts/check_fmi_import_fmpy.py` cross-checks that OpenBMP importer output against FMPy 0.3.26 on the same packaged point-mass FMU; `scripts/check_fmi_reference_smoke.py` downloads the SHA-pinned Modelica Reference-FMUs release, records provenance, imports FMI 3 `Dahlquist.fmu`, `VanDerPol.fmu`, pre-event `BouncingBall.fmu`, `Stair.fmu`, and resource-backed `Resource.fmu`, checks repeated OpenBMP byte stability, and compares final outputs to FMPy. Complete clocked event scheduling and the complete external Reference-FMU matrix remain open.
- **fidelity_tier:** T4
- **depends_on:** [WP-10.1]
...
- **approach:** §3.4. Typed `instantiate_co_simulation`, `enter_initialization_mode`, `exit_initialization_mode`, `terminate`, `free_instance`, `set_float64`/`get_float64`/`set_int32`/`get_int32`/`set_uint64`/`get_uint64`/`set_clock`/`get_clock`/`do_step` over the already-loaded `Fmi3DynamicLibrary`; parse instantiation tokens, value refs, and Clock metadata from stored or deflated `modelDescription.xml` entries (reuse `FmuArchive`); materialize safe `resources/` entries for FMUs that need packaged resource files; infer safe standard FMI binary entries from `modelIdentifier`; master plan carries coupling order + `DoStepOutcome` early-return state; periodic input-clock activation publishes finite positive `constant`/`fixed`/`tunable` input clocks with optional shift at single-FMU macro-step boundaries; optional `get_fmu_state`/`set_fmu_state`/`free_fmu_state` wrappers provide the rollback primitive; `Fmi3SingleFmuMaster` performs typed input publication, stepping, early-return time advancement, typed output reads, and checkpointed early-return rollback for one FMI component; `Fmi3MultiFmuMaster` routes typed connections in Jacobi mode from the previous output cache and in Gauss-Seidel mode from current-step upstream outputs where available, and its predictor/corrector macro-step checkpoints all FMUs, runs a predictor pass, restores FMU states and master clocks to the macro-step start, seeds the corrector from predictor outputs, and leaves the final state at the corrector result; FMPy acts as an independent master for the packaged point-mass importer cross-check and the pinned supported Reference-FMU smoke matrix. Remaining work is complete clocked event scheduling and complete external Reference-FMU validation. `unsafe extern "C"` FFI stays confined to `openbmp-fmi`.
- **acceptance:**
...
- **new_crates:** `openbmp-afts` (L2) — first substrate landed (`no_std`-capable physics-layer monitor; deps only `openbmp-core`/`-state`/`-physics`; FC may host it without breaking the lock)
- **touched:** `crates/openbmp-afts/**(new)`, `crates/openbmp-runner/src/afts.rs`, `crates/openbmp-runner/src/{lib,error,point_mass,rigid_body}.rs`, `crates/openbmp-cli/src/{main.rs,commands/run.rs}`, `crates/openbmp-sil/src/lib.rs`, `crates/openbmp-scenario/src/document.rs`, `crates/openbmp-scenario/src/scenario.rs`, `crates/openbmp-testkit/tests/afts_no_inverse_compile_fail.rs`, `crates/openbmp-testkit/tests/ui/afts_solve_burn_to_reach.rs`, `data/afts/closed-form-iip-tolerance-v1.toml`, `data/afts/provenance.md`, `Cargo.toml`, `.github/workflows/ci.yml`, `docs/HAL.md`, `docs/scenario-format.md`, `requirements.toml`
- **approach:** §3.7. First slice: numeric `r̈ = −GM_earth·r/‖r‖³` to a selected WGS84 sphere or ellipsoid surface with WGS84 symbolic constants, WGS84 uniform-rotation longitude correction, optional deterministic exponential drag, radial and non-radial closed-form conic IIP checks from a provenance-bearing tolerance table, `ContainmentPolygon` point-in-polygon, `ContainmentRule::keep_inside`/`keep_out`, corridor/gate/zone rules, `terminate = monotone-OR(violations)` single latch, schema-v3 `[afts.propagator]` and `[afts]` rule-table validation with optional pinned evidence files, runner-side truth-state observation into `AftsRunReport` without changing kernel stop semantics, SIL/CLI sidecar evidence, native `SilStimulation` in-memory AFTS zone-rule injection, and a trybuild compile-fail tripwire proving no inverse `solve_burn_to_reach` surface exists.
- **acceptance:**
  - radial and non-radial numerically propagated IIP cases match closed-form conic points `≤ 1e-6 rad` (MMS, tolerance table)
...
- **current_state:** Partial implementation. `openbmp-fc::fc_step` now provides a hardware-portable topic-level step facade over `FlightController`: `FcStepInput` carries optional IMU/barometer/GNSS/magnetometer/star-tracker samples plus time/tick; the entry publishes those samples, dispatches one controller tick, captures `ActuatorCommand`, `EffectorCommandSet`, `EngineDemand`, `EngineCommandSet`, `FdirStatus`, `FailsafeFlags`, and `VehicleStatus`, and fails closed when a required input/output topic is not registered. `fc_step_with_timing_observer` preserves the same contract for instrumented host runs. `openbmp-bridge` now has a `no_std` packet/codec/lockstep-validator surface for `thumbv7em-none-eabihf` so target firmware can reuse the same postcard frame schema without host transports, and `encode_into`/`frame_into` support caller-owned target buffers. `openbmp-pil` adds a no-std-capable target shim that decodes bridge `SensorPacket` bytes or length-prefixed frames, adapts them into `FcStepInput`, calls `fc_step`, converts captured effector/engine topics into `ActuatorCommandPacket`, encodes command bytes into caller-provided output storage, exposes a `repr(C)` `PilMailbox` with status/error codes plus `PilMailboxLayout` offsets for downstream GDB-symbol wrappers, exposes a `PilFrameIo` read-step-write loop for downstream complete-frame UART/semihosting wrappers, exposes a fixed-buffer `PilFramePump` for UART-like byte streams that waits for complete frames and preserves queued/pending frames, exposes `poll_byte_stream_endpoint_once` over `PilByteIo` so a target poll loop can read one chunk, drive the pump, write a completed command frame, and retry pending output after write failure, exposes safe `PilFirmware` and `PilFirmwareSymbolContract` target-wrapper surfaces around one controller/mailbox/pump/read-scratch set, and exposes `PilWcetBudget` / `PilWcetReport` integer cycle-approximate deadline evidence helpers. `openbmp-runner::FcRunner::step_bridge_packet` adapts `SensorPacket` into `FcStepInput`, calls the facade, and returns `ActuatorCommandPacket` from captured effector/engine command topics outside `openbmp-fc`; the runner unit gate `pil_framed_endpoint_matches_runner_bridge_packet_adapter` proves the framed PIL endpoint produces the same decoded command packet and encoded command bytes as the runner adapter for the same sensor packet under the host oracle. `openbmp-runner::pil::RenodePilPlan` now renders and validates a deterministic socket-UART Renode `.resc` plan with telnet negotiation disabled, an optional GDB server, and a microsecond global-quantum setting for a future target ELF; `GdbPilSymbolBinding` resolves mailbox and step-entry addresses from target ELF32/ELF64 symtab/dynsym bytes or files; `GdbPilMailboxPlan` maps the resolved target mailbox address into deterministic GDB Remote Serial Protocol read/write/breakpoint packets and decodes protocol/status/error/length/command target reads; `GdbRspSession` drives checked RSP packet exchange over caller-owned streams with ack/checksum handling, target-error handling, memory reads/writes, breakpoint insert/remove, continue, mailbox sensor loading, and mailbox status/command reads; and `PilFrameStream`/`RenodeSocketPilClient` provide the host raw-frame socket client that sends length-prefixed sensor packets, receives length-prefixed actuator-command packets, enforces payload limits, and fails closed on step/time mismatches. Unit tests cover input publication, dispatch, output capture, missing-topic failure, timing-observer dispatch, bridge-packet conversion, PIL byte/mailbox/framed-endpoint/frame-pump/byte-poll/firmware-wrapper/WCET-budget conversion, ELF symbol resolution, GDB mailbox/RSP planning and stream-session mechanics, host raw-frame socket client behavior, host-oracle framed-endpoint equivalence, Renode script rendering/validation, bridge codec/lockstep behavior, and the embedded no-default bridge/PIL checks. No downstream-exported linked target image, live Renode target execution/equivalence gate, real UART driver, or measured WCET artifact exists yet.
- **fidelity_tier:** T5
- **depends_on:** [WP-10.1, WP-10.2]
- **new_crates:** `openbmp-pil` (target-checkable byte/mailbox/framed-endpoint/frame-pump/byte-poll shim, safe firmware wrapper, and integer WCET budget helper outside `openbmp-fc`; a linked target image plus live GDB/UART/Renode transport binding remain future work)
- **touched:** `crates/openbmp-fc/src/lib.rs`, `crates/openbmp-fc/src/step.rs`, `crates/openbmp-bridge/{Cargo.toml,src/lib.rs,src/codec.rs,src/packet.rs,src/lockstep.rs,src/error.rs}`, `crates/openbmp-pil/{Cargo.toml,src/lib.rs}`, `crates/openbmp-runner/src/fc.rs`, `crates/openbmp-runner/src/fc_bridge.rs`, `crates/openbmp-runner/src/pil.rs`; remaining target work will add firmware bin/runtime wrapper, concrete Renode coupler execution, and `tests/fixtures/renode/**` target-run evidence/provenance
- **approach:** §3.5. First slices factor `fc_step(FcStepInput) -> FcStepOutput` inside `openbmp-fc` using only hardware-portable topic types, split `openbmp-bridge` so packet/codec/validators are target-compatible while host transports/fault/XIL surfaces remain `std`-gated, route host bridge packets through `FcRunner::step_bridge_packet` outside the FC crate, add `openbmp-pil` as the no-std byte/mailbox/framed-endpoint/frame-pump/byte-poll shim around `fc_step`, expose concrete mailbox layout offsets and advisory symbol names for known-symbol flows, add a safe target firmware wrapper around controller/mailbox/pump/read scratch, add integer instruction-count × CPI WCET budget helpers, add the deterministic Renode socket-UART `.resc` plan renderer, add target ELF symbol resolution for mailbox/step-entry binding, add the GDB Remote Serial Protocol mailbox planner plus tested stream-session mechanics, and add the host raw-frame socket client for that UART stream. Remaining work is a downstream-exported linked target image, then live Renode GDB-stub or virtual-UART target coupling reusing `openbmp-bridge` frame/codec + `Transport`. Renode `.resc` sets quantum + `sync` for deterministic virtual time. Measured WCET evidence still needs a real instruction count or cycle counter source.
- **acceptance:**
