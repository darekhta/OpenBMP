...
6. **Deferred until validated.** New variants fail closed with a typed
   ...
   validation evidence. PR1 `jettison_stage` is the first implemented slice
   and is limited to the documented rigid-body gravity-only envelope.
...
- [`roadmap.md`](roadmap.md): mark the PR1 gravity-only separation slice as
  shipped and keep profile phases, ascent reference, per-body force ownership,
  and range-safety footprint as Deferred entries.
