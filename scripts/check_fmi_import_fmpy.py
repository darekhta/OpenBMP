#!/usr/bin/env python3
"""Cross-check OpenBMP's FMI importer against FMPy on the point-mass FMU."""

from __future__ import annotations

import platform
import subprocess
import sys
import tempfile
import tomllib
from pathlib import Path


TRACE_CASE = "exported-fmi-point-mass-trace"


def main() -> int:
    root = Path(__file__).resolve().parents[1]
    try:
        from fmpy import read_model_description, simulate_fmu
    except ImportError as exc:
        print(
            "fmi import fmpy: FMPy==0.3.26 is required; install it with "
            "python3 -m pip install FMPy==0.3.26",
            file=sys.stderr,
        )
        print(f"fmi import fmpy: import failed: {exc}", file=sys.stderr)
        return 1

    tolerance = load_trace_tolerance(root)
    run(root, ["cargo", "build", "-p", "openbmp-fmi", "--lib", "--locked"])
    library_path = current_debug_library(root)

    with tempfile.TemporaryDirectory(prefix="openbmp-fmi-import-fmpy-") as tmp:
        tmp_path = Path(tmp)
        fmu_path = tmp_path / "openbmp_point_mass.fmu"
        run(
            root,
            [
                "cargo",
                "run",
                "--quiet",
                "--locked",
                "-p",
                "openbmp-fmi",
                "--example",
                "write_point_mass_fmu",
                "--",
                str(fmu_path),
                str(library_path),
            ],
        )

        description = read_model_description(fmu_path, validate=True)
        if description.fmiVersion != "3.0":
            raise CheckError(f"expected FMI 3.0 model, got {description.fmiVersion}")
        if description.coSimulation is None:
            raise CheckError("generated FMU does not declare CoSimulation")

        openbmp = run_importer(root, tmp_path, fmu_path, tolerance)
        fmpy = run_fmpy(fmu_path, tolerance)
        check_samples_match(parse_csv_sample(openbmp.stdout), fmpy, tolerance)

    print("fmi import fmpy: OpenBMP importer matches FMPy point-mass trace")
    return 0


class CheckError(RuntimeError):
    """OpenBMP-vs-FMPy importer cross-check failed."""


def run_importer(
    root: Path,
    tmp_path: Path,
    fmu_path: Path,
    tolerance: dict[str, float | int | str],
) -> subprocess.CompletedProcess[str]:
    return run(
        root,
        [
            "cargo",
            "run",
            "--quiet",
            "--locked",
            "-p",
            "openbmp-fmi",
            "--example",
            "import_point_mass_fmu",
            "--",
            str(fmu_path),
            str(tmp_path / "materialized-openbmp"),
            str(tolerance["throttle"]),
            str(tolerance["step_s"]),
            str(tolerance["steps"]),
        ],
    )


def run_fmpy(fmu_path: Path, tolerance: dict[str, float | int | str]) -> dict[str, float | int]:
    from fmpy import simulate_fmu

    stop_time = float(tolerance["step_s"]) * int(tolerance["steps"])
    result = simulate_fmu(
        str(fmu_path),
        start_time=0.0,
        stop_time=stop_time,
        output_interval=float(tolerance["step_s"]),
        output=["altitude", "velocity", "step"],
        start_values={"throttle": float(tolerance["throttle"])},
        validate=True,
    )
    final = result[-1]
    return {
        "time": float(final["time"]),
        "altitude": float(final["altitude"]),
        "velocity": float(final["velocity"]),
        "step": int(final["step"]),
    }


def run(root: Path, command: list[str]) -> subprocess.CompletedProcess[str]:
    result = subprocess.run(
        command,
        cwd=root,
        text=True,
        capture_output=True,
        check=False,
    )
    if result.returncode != 0:
        sys.stdout.write(result.stdout)
        sys.stderr.write(result.stderr)
        raise SystemExit(result.returncode)
    return result


def current_debug_library(root: Path) -> Path:
    system = platform.system()
    if system == "Darwin":
        name = "libopenbmp_fmi.dylib"
    elif system == "Linux":
        name = "libopenbmp_fmi.so"
    elif system == "Windows":
        name = "openbmp_fmi.dll"
    else:
        raise CheckError(f"unsupported FMI import/FMPy smoke platform {system}")

    path = root / "target" / "debug" / name
    if not path.is_file():
        raise CheckError(f"could not find built openbmp-fmi cdylib at {path}")
    return path


def load_trace_tolerance(root: Path) -> dict[str, float | int | str]:
    path = root / "crates" / "openbmp-fmi" / "tests" / "expected" / "export-point-mass-trace.toml"
    with path.open("rb") as fh:
        tolerance = tomllib.load(fh)
    if tolerance.get("case") != TRACE_CASE:
        raise CheckError(f"unexpected trace tolerance case {tolerance.get('case')!r}")
    return tolerance


def parse_csv_sample(stdout: str) -> dict[str, float | int]:
    lines = [line.strip() for line in stdout.splitlines() if line.strip()]
    if len(lines) != 2:
        raise CheckError(f"expected two CSV lines from importer example, got {len(lines)}")
    header = lines[0].split(",")
    if header != ["time_s", "altitude_m", "velocity_m_s", "step"]:
        raise CheckError(f"unexpected CSV header {header!r}")
    fields = lines[1].split(",")
    if len(fields) != 4:
        raise CheckError(f"expected four CSV fields, got {len(fields)}")
    return {
        "time": float(fields[0]),
        "altitude": float(fields[1]),
        "velocity": float(fields[2]),
        "step": int(fields[3]),
    }


def check_samples_match(
    openbmp: dict[str, float | int],
    fmpy: dict[str, float | int],
    tolerance: dict[str, float | int | str],
) -> None:
    assert_close("time", float(openbmp["time"]), float(fmpy["time"]), tolerance)
    assert_close("altitude", float(openbmp["altitude"]), float(fmpy["altitude"]), tolerance)
    assert_close("velocity", float(openbmp["velocity"]), float(fmpy["velocity"]), tolerance)
    openbmp_step = int(openbmp["step"])
    fmpy_step = int(fmpy["step"])
    if openbmp_step != fmpy_step:
        raise CheckError(f"step: OpenBMP {openbmp_step}, FMPy {fmpy_step}")


def assert_close(
    metric: str,
    actual: float,
    expected: float,
    tolerance: dict[str, float | int | str],
) -> None:
    diff = abs(actual - expected)
    bound = max(
        float(tolerance["absolute_tolerance"]),
        float(tolerance["relative_tolerance"]) * max(abs(expected), 1.0),
    )
    if diff > bound:
        raise CheckError(
            f"{metric}: OpenBMP {actual:e}, FMPy {expected:e}, diff {diff:e}, bound {bound:e}"
        )


if __name__ == "__main__":
    raise SystemExit(main())
