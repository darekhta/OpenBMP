...
//! filter for attitude-only estimation. A real sigma-point UKF is a
//! Phase 4.C deferral: the academic-tier scaffold that previously
//! stood in this file propagated only the mean and inflated the
//! diagonal — that is an EKF, not a UKF — so the type was deleted
//! rather than left to misrepresent itself.
//!
...
use crate::magnetic::{EarthDipoleField, MagneticFieldModel};
...
/// Gravity model — abstracts the gravitational acceleration vector
/// at a given inertial position. Implementations may range from
/// constant flat-Earth (`ConstantGravityZ`) to spherical-harmonic
/// gravity.
pub trait GravityModel: std::fmt::Debug {
    /// Returns gravitational acceleration in ECI (m/s²).
    fn at(&self, position_eci_m: Vector3<f64>) -> Vector3<f64>;
}

/// Constant gravity vector along the ECI −z axis. Default −9.81 m/s².
/// Suitable for short-duration academic scenarios on a flat-Earth
/// reference frame.
#[derive(Copy, Clone, Debug)]
pub struct ConstantGravityZ {
    /// Gravitational acceleration along ECI z (m/s²). Negative for
    /// "down".
    pub g_z_m_s2: f64,
}

impl Default for ConstantGravityZ {
    fn default() -> Self {
        Self { g_z_m_s2: -9.81 }
    }
}

impl GravityModel for ConstantGravityZ {
    fn at(&self, _position_eci_m: Vector3<f64>) -> Vector3<f64> {
        Vector3::new(0.0, 0.0, self.g_z_m_s2)
    }
}
...
pub struct EkfParams {
    ...
    /// Innovation-gate threshold (chi-square ratio) above which a
...
pub struct Ekf {
    ...
    mag_field: Box<dyn MagneticFieldModel>,
}
...
impl Ekf {
    ...
    /// gravity model (`ConstantGravityZ::default`), and the academic-
    ...
    pub fn new(params: EkfParams) -> Self {
        ...
        Self {
            ...
            last_chi2_mag: 0.0,
            initialized: false,
            gravity: Box::new(ConstantGravityZ::default()),
    ...
    #[must_use]
    pub fn with_gravity_model<G: GravityModel + 'static>(mut self, gravity: G) -> Self {
    ...
    #[must_use]
    pub fn with_mag_field_model<M: MagneticFieldModel + 'static>(mut self, mag: M) -> Self {
...
impl Estimator for Ekf {
    ...
    fn predict(&mut self, dt: f64) -> Result<(), EstimatorError> {
        ...
        let g_eci = self.gravity.at(self.pos_eci);
        ...
        let q_diag = SVector::<f64, 15>::from_iterator((0..15).map(|i| match i {
            ...
            9..=11 => self.params.sigma_w_gyro_bias * self.params.sigma_w_gyro_bias * dt,
            12..=14 => self.params.sigma_w_accel_bias * self.params.sigma_w_accel_bias * dt,
            _ => 0.0,
        }));
        for i in 0..15 {
            self.p[(i, i)] += q_diag[i];
    ...
    fn update_baro(&mut self, sample: &BarometerSample) -> Result<(), EstimatorError> {
        ...
        // US Standard Atmosphere 1976 troposphere segment, then form
        // a 1-D position-z innovation. Re-implementing the formula
        // inline keeps `openbmp-fc` HAL-portable (no `openbmp-env`
        // dependency).
        let measured_alt_m = pressure_altitude_us_std_1976_m(sample.pressure_pa - sample.bias_pa);
    ...
    fn update_mag(&mut self, sample: &MagnetometerSample) -> Result<(), EstimatorError> {
        ...
        // Predict body-frame field by rotating the ECI dipole field
        // into body via R(q)ᵀ.
        let r_eci_to_body = self.q_body_to_eci.to_rotation_matrix().transpose();
        let predicted = r_eci_to_body * self.mag_field.field_eci_nt(self.pos_eci);
        let innovation = measured - predicted;

        // 3-D measurement; H couples into the 3-axis attitude error
        // block (state indices 6..9). The Jacobian of the rotated
        // field with respect to a body-frame attitude error δθ is
        // [predicted]× (small-angle skew-symmetric).
...
fn quaternion_from_axis_angle(axis_angle: Vector3<f64>) -> UnitQuaternion<f64> {
    ...
    if mag < 1e-12 {
        ...
        let axis = axis_angle / mag;
        UnitQuaternion::from_scaled_axis(axis * mag)
    }
}
...
/// Inverts the US Standard Atmosphere 1976 troposphere relation
/// `p(h) = p0 · (1 + L · h / T0)^(-g · M / (R · L))` to recover
/// geopotential altitude (m) from a static pressure (Pa).
///
/// Constants:
/// - `p0` = 101 325 Pa (sea-level static pressure)
/// - `T0` = 288.15 K (sea-level standard temperature)
/// - `L`  = −0.0065 K/m (tropospheric lapse rate)
/// - `g`  = 9.80665 m/s² (standard gravity)
/// - `M`  = 0.028 964 4 kg/mol (mean molar mass of dry air)
/// - `R`  = 8.314 462 618 J/(mol·K)
///
/// Valid for the troposphere segment (0 … 11 km). At pressures below
/// the 11-km tropopause cutoff (≈22 632 Pa) the troposphere formula
/// is no longer accurate; the result is clamped to the
/// tropopause altitude rather than extrapolated.
fn pressure_altitude_us_std_1976_m(pressure_pa: f64) -> f64 {
    ...
    const TROPOPAUSE_PA: f64 = 22_632.06;
    ...
    let exponent_recip = -R * LAPSE / (G * M_AIR);
    let ratio = pressure_pa / P0;
    (T0 / LAPSE) * (ratio.powf(exponent_recip) - 1.0)
}
...
mod tests {
    ...
    fn ekf_predict_advances_position_under_velocity() {
        ...
        let mut ekf =
            Ekf::new(EkfParams::default()).with_gravity_model(ConstantGravityZ { g_z_m_s2: 0.0 });
        ekf.seed(
    ...
    fn ekf_free_fall_matches_analytic_kinematics() {
        ...
        // analytic answer is z = -0.5 · g · t² = -490.5 m; the
        // integrated answer differs by O(dt · g · t) ≈ 4.9 cm.
        assert!(
            (pos.position_eci_m.z - (-490.5)).abs() < 0.05,
            "position z {} not within 5 cm of -490.5 m",
            pos.position_eci_m.z
        );
        assert!(
            (pos.velocity_eci_m_s.z - (-98.1)).abs() < 0.05,
            "velocity z {} not within 5 cm/s of -98.1 m/s",
...
impl Mekf {
    ...
    pub fn new(params: MekfParams) -> Self {
        ...
        Self {
            ...
            last_chi2_mag: 0.0,
            initialized: false,
            reference_position_eci_m: Vector3::new(6_371_000.0, 0.0, 0.0),
            mag_field: Box::new(EarthDipoleField::default()),
        }
    }
...
impl Estimator for Mekf {
    ...
    fn predict(&mut self, dt: f64) -> Result<(), EstimatorError> {
        ...
        for i in 0..3 {
            ...
            self.p[(i + 3, i + 3)] +=
                self.params.sigma_w_gyro_bias * self.params.sigma_w_gyro_bias * dt;
        }
        Ok(())
    }
    ...
    fn update_mag(&mut self, sample: &MagnetometerSample) -> Result<(), EstimatorError> {
        ...
        let measured = sample.field_body_nt - sample.hard_iron_body_nt;
        let r_eci_to_body = self.q_body_to_eci.to_rotation_matrix().transpose();
        let predicted = r_eci_to_body * self.mag_field.field_eci_nt(self.reference_position_eci_m);
        let innovation = measured - predicted;
        ...
        // Apply attitude error (3) + gyro bias (3).
