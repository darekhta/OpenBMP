//! Textbook Kalman-filter consistency examples.
...
#[test]
fn bar_shalom_scalar_update_matches_closed_form_consistency_example() {
    // Bar-Shalom/Li/Kirubarajan §5 textbook scalar update: the
    // posterior of two independent Gaussian information sources has
    // variance (P⁻¹ + R⁻¹)⁻¹ and mean weighted by information.
    let prior_x = 10.0;
    let prior_p = 4.0;
    let z = 12.0;
    let r = 9.0;
...
#[test]
fn bar_shalom_whitened_innovation_has_unit_covariance() {
    let prior_p = 2.0_f64;
    let r = 3.0_f64;
