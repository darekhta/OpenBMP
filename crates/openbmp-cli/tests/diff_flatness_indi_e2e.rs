...
//! across reruns.
//!
//! `scenarios/diff-flatness-figure-eight-indi/scenario.toml` is a
//! sibling of `scenarios/diff-flatness-figure-eight`; the only
//! differences are `rate_loop_kind = "indi"` and the
//! `[fc.autopilot_params.indi]` block declaring INDI's working
//! estimate of body-axis inertia, the synchronised filter cutoff
//! (50 rad/s ≈ 8 Hz, well below the discrete Nyquist boundary of
//! π · 1000 ≈ 3141.6 rad/s for the 1 ms loop), the second-order
//! Butterworth filter shape (Smeur 2016 default), and the outer-loop
//! attitude-to-angular-acceleration P-gain.
//!
...
//! byte-identical Parquet (the synchronised LPF biquads and the
//! finite-difference ω̇ derivation are deterministic).
//!
...
fn tolerance_table_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/expected/diff-flatness-figure-eight-indi.toml")
}
...
fn run_to_parquet(label: &str) -> RunOutput {
    let scenario = workspace_root().join("scenarios/diff-flatness-figure-eight-indi/scenario.toml");
    let temp = tempdir(label);
    let parquet = temp.path().join("diff-flatness-figure-eight-indi.parquet");
    let mut cmd = openbmp();
    cmd.arg("run")
        .arg(&scenario)
        .arg("--output-parquet")
...
#[test]
fn diff_flatness_indi_runs_to_completion() {
    let run = run_to_parquet("indi-runs");
    ...
    assert!(
        max_omega > 1.0e-6,
        "INDI figure-eight must rotate the rigid body; max angular velocity was {max_omega}"
    );
...
#[test]
fn diff_flatness_indi_is_byte_stable_across_reruns() {
    let a = run_to_parquet("indi-rerun-a");
    let b = run_to_parquet("indi-rerun-b");
    let bytes_a = fs::read(&a.parquet).expect("read a");
    let bytes_b = fs::read(&b.parquet).expect("read b");
    assert_eq!(
        bytes_a, bytes_b,
        "two reruns of diff-flatness-figure-eight-indi must produce byte-identical Parquet \
         (the synchronised LPF biquads + finite-difference ω̇ derivation are deterministic)"
    );
