...
//!    `r = 6_778_000 m` — the EGM2008 zonal corrections induce a small
//!    periodic radial perturbation but the orbit does not blow up.
//! 3. Two reruns produce byte-identical Parquet (the gravity model is
//!    deterministic: pure `f64` arithmetic with locked operand order,
//!    no FMA, no system RNG).
//!
//! The math-side correctness of `Egm2008ZonalGravity` itself
//! (degree-2 degeneracy to `J2Gravity`, higher-degree differentiation
//! at non-equatorial points, Legendre-recurrence sanity) is covered
//! by the unit tests in `crates/openbmp-physics/src/gravity.rs`.
...
fn scenario_path() -> PathBuf {
    workspace_root().join("scenarios/leo-orbit-egm2008/scenario.toml")
}
...
#[test]
fn leo_orbit_egm2008_runs_to_completion_within_radius_envelope() {
    let temp = tempdir_for("openbmp_egm2008_run");
    let staged = stage_scenario(temp.path(), "egm2008_run");

    let report = run::run(&staged).expect("scenario run must succeed");

    assert_eq!(report.stop_label, "end-time");
    assert_eq!(report.final_step, 5556);
    ...
    // perturbation; the orbit should not drift more than a few km
    // from the initial circular radius after one nominal period.
...
#[test]
fn leo_orbit_egm2008_is_byte_stable_across_two_runs() {
    let temp = tempdir_for("openbmp_egm2008_bytestable");

    let staged_a = stage_scenario(temp.path(), "egm2008_run_a");
    let staged_b = stage_scenario(temp.path(), "egm2008_run_b");
    ...
    assert_eq!(
        bytes_a, bytes_b,
        "two reruns of leo-orbit-egm2008 must produce byte-identical Parquet \
         (EGM2008 zonal gravity is deterministic with locked operand order)",
    );
}
