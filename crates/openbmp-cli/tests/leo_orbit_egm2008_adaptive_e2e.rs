...
//!    `r = 6_778_000 m` — same envelope as the fixed-RK4 demo,
//!    demonstrating that the adaptive integrator delivers
//!    comparable accuracy under these tolerances (it does not
//!    drift the orbit catastrophically from the closed-form
//!    circular IC).
//! 3. Two reruns on the same platform produce byte-identical
//!    Parquet — `Dopri54Adaptive` is `StateStable`, which is
//!    within-platform bit-stable: the PI step-size search is
//!    deterministic given identical inputs and the `last_h_s` /
//!    `last_err_prev` Cells start from the same `None` seed.
//!
//! The math-side correctness of `Dopri54Adaptive` (5th-order
//! convergence, tableau row-sums, PI-factor monotonicity) is covered
...
fn scenario_path() -> PathBuf {
    workspace_root().join("scenarios/leo-orbit-egm2008-adaptive/scenario.toml")
}
...
#[test]
fn leo_orbit_egm2008_adaptive_runs_to_completion_within_radius_envelope() {
    let temp = tempdir_for("openbmp_egm2008_adaptive_run");
    let staged = stage_scenario(temp.path(), "egm2008_adaptive_run");
    ...
    assert_eq!(
        ...
        "outer kernel step count is set by [time] dt_s + stop_s, \
         independent of adaptive sub-stepping"
    );
    ...
    assert!(
        ...
        "final radius {r_final:.1} m drifted {radial_error:.1} m from initial \
         ...
         + Dopri54Adaptive at rtol = 1e-9, atol = 1e-12)",
    );
}

#[test]
fn leo_orbit_egm2008_adaptive_is_byte_stable_across_two_runs_within_platform() {
    let temp = tempdir_for("openbmp_egm2008_adaptive_bytestable");

    let staged_a = stage_scenario(temp.path(), "egm2008_adaptive_run_a");
    let staged_b = stage_scenario(temp.path(), "egm2008_adaptive_run_b");
    ...
    assert_eq!(
        bytes_a, bytes_b,
        "two reruns of leo-orbit-egm2008-adaptive on the same platform must \
         produce byte-identical Parquet — Dopri54Adaptive is StateStable, \
         which is within-platform bit-stable: the PI step-size search is \
         deterministic and last_h_s / last_err_prev start from None on each run",
    );
}
