...
//!
//! `scenarios/diff-flatness-figure-eight-mpc/scenario.toml` is a
//! sibling of `scenarios/diff-flatness-figure-eight`; the only
//! differences are `attitude_loop_kind = "mpc"` and the
//! `[fc.autopilot_params.attitude_mpc]` block. The runner
//! constructs a `RecedingHorizonAttitudeMpc` at scenario load using
//! the configured params and the loop step `time.dt_s`. Clarabel is
//! configured with the deterministic settings shared with Phase
//! 4.C's `solve_attitude_box_qp`, so the QP solution is
//! bit-identical between runs on the reference platform.
//!
...
fn tolerance_table_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/expected/diff-flatness-figure-eight-mpc.toml")
}
...
fn run_to_parquet(label: &str) -> RunOutput {
    let scenario = workspace_root().join("scenarios/diff-flatness-figure-eight-mpc/scenario.toml");
    let temp = tempdir(label);
    let parquet = temp.path().join("diff-flatness-figure-eight-mpc.parquet");
    let mut cmd = openbmp();
    cmd.arg("run")
        .arg(&scenario)
        .arg("--output-parquet")
...
#[test]
fn diff_flatness_mpc_runs_to_completion() {
    let run = run_to_parquet("mpc-runs");
    ...
    assert!(
        max_omega > 1.0e-6,
        "MPC figure-eight must rotate the rigid body; max angular velocity was {max_omega}"
    );
...
#[test]
fn diff_flatness_mpc_is_byte_stable_across_reruns() {
    let a = run_to_parquet("mpc-rerun-a");
    let b = run_to_parquet("mpc-rerun-b");
    let bytes_a = fs::read(&a.parquet).expect("read a");
    let bytes_b = fs::read(&b.parquet).expect("read b");
    assert_eq!(
        bytes_a, bytes_b,
        "two reruns of diff-flatness-figure-eight-mpc must produce byte-identical Parquet \
         (Clarabel runs with deterministic settings; the QP solution is bit-stable across runs)"
    );
