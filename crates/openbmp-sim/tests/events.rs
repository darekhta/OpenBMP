...
fn scalars(time_s: f64, alt: f64, v_z: f64, mf: f64, q: f64) -> EventScalars {
    EventScalars {
        time_s,
        altitude_m: alt,
...
fn at_time_does_not_fire_on_step_zero() {
    ...
    let s = step(scalars(0.0, 0.0, 100.0, 1.0, 0.0), None);
    assert!(!BuiltInEventTrigger::AtTime { time_s: 0.0 }.fired(&s, ANY_TIME, ANY_STEP));
}

#[test]
fn at_time_fires_on_first_crossing() {
    let prev = scalars(0.99, 0.0, 0.0, 1.0, 0.0);
    let curr = scalars(1.00, 0.0, 0.0, 1.0, 0.0);
...
fn at_time_does_not_re_fire_after_crossing() {
    let prev = scalars(1.5, 0.0, 0.0, 1.0, 0.0);
    let curr = scalars(2.0, 0.0, 0.0, 1.0, 0.0);
    let s = step(curr, Some(prev));
    assert!(!BuiltInEventTrigger::AtTime { time_s: 1.0 }.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_time_does_not_fire_before_threshold() {
    let prev = scalars(0.5, 0.0, 0.0, 1.0, 0.0);
    let curr = scalars(0.6, 0.0, 0.0, 1.0, 0.0);
    let s = step(curr, Some(prev));
    assert!(!BuiltInEventTrigger::AtTime { time_s: 1.0 }.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_altitude_ascending_fires_on_upward_crossing() {
    let prev = scalars(1.0, 95.0, 50.0, 1.0, 0.0);
    let curr = scalars(1.1, 105.0, 50.0, 1.0, 0.0);
    let s = step(curr, Some(prev));
    assert!(
...
fn at_altitude_ascending_does_not_fire_going_down() {
    let prev = scalars(5.0, 105.0, -10.0, 1.0, 0.0);
    let curr = scalars(5.1, 95.0, -10.0, 1.0, 0.0);
    let s = step(curr, Some(prev));
    assert!(
        !BuiltInEventTrigger::AtAltitudeAscending { meters: 100.0 }.fired(&s, ANY_TIME, ANY_STEP)
    );
...
fn at_apogee_fires_on_velocity_sign_flip() {
    let prev = scalars(5.0, 150.0, 5.0, 0.5, 0.0);
    let curr = scalars(5.1, 151.0, -1.0, 0.5, 0.0);
    let s = step(curr, Some(prev));
...
fn at_apogee_does_not_fire_during_ascent() {
    let prev = scalars(1.0, 50.0, 100.0, 1.0, 0.0);
    let curr = scalars(1.1, 60.0, 95.0, 1.0, 0.0);
    let s = step(curr, Some(prev));
    assert!(!BuiltInEventTrigger::AtApogee.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_apogee_does_not_fire_during_descent() {
    let prev = scalars(10.0, 100.0, -10.0, 0.5, 0.0);
    let curr = scalars(10.1, 99.0, -11.0, 0.5, 0.0);
    let s = step(curr, Some(prev));
    assert!(!BuiltInEventTrigger::AtApogee.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_apogee_fires_when_velocity_reaches_zero() {
    ...
    let prev = scalars(5.0, 150.0, 0.5, 0.5, 0.0);
    let curr = scalars(5.1, 150.5, 0.0, 0.5, 0.0);
    let s = step(curr, Some(prev));
    assert!(BuiltInEventTrigger::AtApogee.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_mass_fraction_fires_on_threshold_cross() {
    let prev = scalars(2.0, 100.0, 50.0, 0.51, 0.0);
    let curr = scalars(2.1, 105.0, 49.0, 0.49, 0.0);
    let s = step(curr, Some(prev));
...
fn at_mass_fraction_does_not_fire_above_threshold() {
    let prev = scalars(1.0, 50.0, 100.0, 0.9, 0.0);
    let curr = scalars(1.1, 60.0, 95.0, 0.85, 0.0);
    let s = step(curr, Some(prev));
    assert!(!BuiltInEventTrigger::AtMassFraction { remaining: 0.5 }.fired(&s, ANY_TIME, ANY_STEP));
}
...
fn at_dynamic_pressure_rising_edge_fires() {
    let prev = scalars(1.0, 50.0, 100.0, 1.0, 9_500.0);
    let curr = scalars(1.1, 60.0, 100.0, 1.0, 10_500.0);
    let s = step(curr, Some(prev));
    assert!(
...
fn at_dynamic_pressure_rising_edge_ignores_falling_crossing() {
    ...
    let prev = scalars(8.0, 200.0, -10.0, 0.4, 10_500.0);
    let curr = scalars(8.1, 199.0, -11.0, 0.4, 9_500.0);
    let s = step(curr, Some(prev));
    assert!(
        !BuiltInEventTrigger::AtDynamicPressure {
            pa: 10_000.0,
