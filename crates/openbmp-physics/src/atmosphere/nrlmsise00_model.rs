//! Pure-Rust NRLMSISE-00 coefficient evaluator.
//!
//! This module is an OpenBMP-local evaluator for the public NRLMSISE-00
//! coefficient model. The coefficient tables live in
//! `nrlmsise00_coefficients.rs` and are generated from the NASA/CCMC
//! ModelWeb archive copy of `nrlmsise-00_data.c`. The evaluator structure
//! is adapted from the MIT-licensed `tobari` 0.2.0 Rust implementation
//! (`https://github.com/sksat/orts`) and kept private to the atmosphere
//! module so OpenBMP owns the public API and validation surface.
//! Third-party notice: portions adapted from `tobari` 0.2.0, authors
//! sksat, license MIT.

// Keep this private evaluator structurally close to the NRLMSISE-00
// equations and source-port lineage; local style rewrites would make
// future source parity audits harder than the lints would help.
#![allow(
...
use super::Nrlmsise00LowLevelInput;
use super::nrlmsise00_coefficients::*;
...
fn density_temperature_profile(
    ...
    // d(1/T)/dx = d(1/T)/dzeta * dzeta/dx = d(1/T)/dzeta * zgdif
    // d(1/T)/dzeta = -(1/T^2) * dT/dzeta
...
// --- Main computation (GTS7/GTD7 equivalent) ---

/// Full NRLMSISE-00 computation.
///
/// Returns (densities\[9\], temperature_exo, temperature_alt).
/// Number densities are in `cm^-3`; `densities[5]` is total mass
/// density in `g/cm^3`.
pub fn compute(input: &Nrlmsise00LowLevelInput) -> ([f64; 9], f64, f64) {
    ...
    let g28 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[2], input, &sw, &plg);
    ...
    // --- He ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[0], input, &sw, &plg);
    ...
    // --- O ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[1], input, &sw, &plg);
    ...
    // --- O2 ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[4], input, &sw, &plg);
    ...
    // --- Ar ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[5], input, &sw, &plg);
    ...
    // --- H ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[6], input, &sw, &plg);
    ...
    // --- N ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[7], input, &sw, &plg);
    ...
    // --- Anomalous O ---
    {
        let g1 = sw[21] * geographic_variation(&DENSITY_COEFFICIENTS[8], input, &sw, &plg);
    ...
    // --- Total mass density [g/cm^3], matching NRLMSISE-00 gtd7 ---
