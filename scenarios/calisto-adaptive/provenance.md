...
Synthetic. The scenario is the Phase-5.D.5 end-to-end demo for the
...
with the §5.D.5 per-component Hairer-Nørsett-Wanner Vol I §II.4 RMS
error norm).
...
Phase-3.11 RocketPy cross-tool validation scenario). See that
...
These tolerances are looser than the Phase-5.D.4 LEO-orbit demo
...
## Per-component error norm (Phase-5.D.5)
...
The §5.D.4 scalar form `||e||₂ / (atol + rtol · ||y||₂)` would let
...
deterministically. The Phase-5.D.5 e2e test
...
- the apogee stays within ±2 % of the RocketPy fixed-RK4 cross-tool
  baseline (3 349 m AGL) — the same envelope as the Phase-3.11
