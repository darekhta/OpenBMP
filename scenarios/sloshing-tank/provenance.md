...
Canonical OpenBMP provenance record for the slosh
...
source_title:     >-
  Exit-criterion scenario. Rigid-body 1000 kg vehicle
...
source_authors:   OpenBMP (Dmitri Arekhta) for the slosh
                  exit-criterion case
source_id:        Synthetic OpenBMP slosh fixture
publication_date: 2026-04-28
methodology_reference: >-
  `docs/scenario-format.md` § Tanks and slosh;
  slosh dynamics per Abramson 1966 NASA SP-106 §7.4
  (cylindrical-tank antisymmetric fundamental mode); the runner-side
  TankRack mirrors the EngineRack pattern.
...
- Vehicle dimensions, inertia, engine thrust, and tank size are
  ...
  known mass overcount (`TankRackMassAdapter` adds
...
- Propellant is a synthetic low-density liquid (250 kg/m³), chosen so
  ...
  height used by the slosh-frequency calculation. The scenario uses
  no fielded propellant data per `docs/safety-boundaries.md`.
...
- All byte-stability gates (analytic-toy, Niskanen
  point + rigid, effector elevon, effector-
  aero, octaweb four-engine, sloshing tank)
  remain green.

### Limitations exercised by this scenario

- Drain decoupled from engine cluster: tank's
  `drain_rate_kg_per_s = 0.0`. The engine consumes its own
  internal propellant accounting; the
  tank's fluid mass is constant.
- Mass overcount: `EngineClusterMassAdapter` publishes
  ...
  growing.
- The kernel-side rigid mass adapter for engine clusters is
  `ConstantMassRigid` over assembly dry properties; the
  `RigidMotorMassAdapter` family does not
  participate. Slosh inertia delta is published in `TankSnapshot`
  but is not consumed by the rigid mass-properties model.
...
Per-tank slosh-state telemetry channels are not emitted — they
would require a tank-specific channel set in the runner.
